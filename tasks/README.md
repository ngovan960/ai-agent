...
├── Retry Tracking Service (v4.2 — max retries, can_retry, stats)
├── Audit Service (v4.2 — query logs, CSV export, compliance)
...
| **Pydantic Schemas** | project.py, module.py, task.py, validation.py, retry_audit.py | CRUD DTOs, validation gate DTOs, retry/audit DTOs |
| **Services** | projects.py, modules.py, tasks.py, validation.py, context_builder.py, stuck_task_detector.py, notification_service.py, retry_audit_service.py | CRUD + validation + context builder + stuck detection + notifications + retry/audit |
| **Routers** | projects.py, modules.py, tasks.py, validation.py, retry_audit.py | REST API + validation + retry/audit endpoints |
